...
class SalesOrderModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        $data['leftJoin']['(SELECT pm.item_id,SUM(pd.stored_qty) AS total_stored, SUM(pd.rej_qty) AS total_rej FROM prc_detail pd LEFT JOIN prc_master pm ON pm.id = pd.prc_id AND pm.is_delete = 0 AND pm.prc_type = 1 AND pm.status = 2 WHERE pd.is_delete = 0 GROUP BY pm.item_id) AS pd'] = "pd.item_id = so_trans.item_id";
